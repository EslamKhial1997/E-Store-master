...
function ChatTest() {
  ...
  useEffect(() => {
    socket.on("receiveMessage","admin");
  ...
  const sendMessage = () => {
    ...
    setUsers((prevMessages) => [...prevMessages, newMessage.receiver.includes(prevMessages) ? "" : newMessage]);
    ...
    <div>
      {!isRegistered ? (
        <div>
          ...
          />
          <label className="bg-info  d-block">
            Admin:
            <input
              type="checkbox"
              checked={isAdmin}
          ...
          <button onClick={registerUser} className="btn btn-danger d-block">Register</button>
        </div>
      ) : (

...
const Chat = () => {
  ...
  const { data, dataById } = useSelector((state) => state.StoreSliceUser);
  ...
  useEffect(() => {
    ...
    dispatch(getUser("65b565df3247709928b1a1ff", token));
  ...
  const sendMessage = () => {
    const socket = socketIOClient("http://localhost:3000");
    const newMessage = { from: username, to: receiver, message };

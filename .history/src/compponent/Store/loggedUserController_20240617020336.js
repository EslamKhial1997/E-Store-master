...
export const getLoggedData = createAsyncThunk(
  "auth",
  async (data, thunkAPI) => {
...
const LoggedController = createSlice({
  name: "auth",
  initialState,
  ...
  extraReducers: {
    [getLoggedData.pending]: (state) => {
      state.isLoading = true;
      state.isError = false;
    },
    [getLoggedData.fulfilled]: (state, {payload}) => {
      state.isLoading = false;
      state.isError = false;
      state.data = payload.data;
    },
    [getLoggedData.rejected]: (state) => {
      state.isLoading = false;
      state.isError = true;
    },
    [login.pending]: (state) => {
...
export const StoreSliceUserLogged = LoggedController.reducer;
export const {
  setOpen,
  setCreate,
